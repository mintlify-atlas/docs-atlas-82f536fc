...
|---|---|---|
| `CONFIG_BUG_ON_DATA_CORRUPTION=y` | any | `BUG_ON` → immediate host kernel panic |
| `CONFIG_BUG_ON_DATA_CORRUPTION=n` | — | `WARN_ON_ONCE` + `kvm_vm_bugged()` — guest VM is killed, host survives |

<Warning>
`CONFIG_BUG_ON_DATA_CORRUPTION=y` is the **default in RHEL** and many other enterprise distributions. Combined with the default `panic_on_oops=1`, a single successful race triggers an immediate, unrecoverable host kernel panic. Every guest on the physical host is simultaneously brought down.
</Warning>
