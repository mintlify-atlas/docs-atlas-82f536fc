...
Yes, disabling nested virtualization on the host removes the attack surface for Januscape. The shadow MMU path that contains the bug — `kvm_mmu_get_child_sp()` being called via the `ept_page_fault` handler (and the corresponding AMD nested NPT fetch path) — is only reached when a guest is running its own nested guest. If nested virtualization is not exposed to guests, the vulnerable code is unreachable.
